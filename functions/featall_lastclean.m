function featall = featall_lastclean (pathfhdd, ividclft, frame_index, frame_index1, im)

if exist([pathfhdd,num2str(ividclft),'_pad.mat'],'file')
    load ([pathfhdd,num2str(ividclft),'_pad.mat'])
    ...
    if size(featall,1)<size(frame_index,1)
        if exist('featall_pad','var')
            ...
            save([pathfhdd,num2str(ividclft),'_pad.mat'],'featall_pad','-append','-v7.3');
            display([pathd,'output/features/all/',num2str(ivid),'.mat']);
        end
    end
end

for ifr = size(frame_index1,1)+1 : size(frame_index,1)
    offset =  ifr - size(frame_index1,1) ;
    featall(ifr,:)= featall(size(frame_index1,1) - offset ,:);
end
