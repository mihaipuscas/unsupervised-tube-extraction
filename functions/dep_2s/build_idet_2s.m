function Idet1 = build_idet_2s (ivid, frame_index, pathout, nrvals)
...
for i=1:length(frame_index)
    ...
    if i>1
        for kd=i-1:i
            B_det_single_traj=[];
            load([pathout,'cache/',num2str(ivid),'/',num2str(kd),'_det_traj.mat']);
        ...
        save([pathout,'cache/',num2str(ivid),'/',num2str(i),'_idet.mat'],'Idet_single','i')
    end
end