...
// Modified to only output the the trajectories' absolute coordinates.
...
int main(int argc, char** argv)
{
    ...
    std::ofstream feat1,feat2;
    //feat1.open ("out.featuresstas.txt");
    //feat2.open ("out.impdtstas.txt");
...
//	fprintf(stderr, "video size, length: %d, width: %d, height: %d\n", seqInfo.length, seqInfo.width, seqInfo.height);
	...
	Mat image, prev_grey, grey, black;
	...
	while(true) {
		Mat frame;
		int i, j, c, contor_cadre;
		char liniec[100], linieb[100];
		...
		if(frame_num == start_frame) {
			...
			black.create(frame.size(), CV_8UC3);
        ...
        s_n<<std::setw(5) << std::setfill('0')<<frame_num<< "_surf.txt";
        std::string surf_name = s_n.str();
        //surf_file.open (surf_name.c_str());

        /*surf_file<<"\n";
		for (i=0 ; i<pts_surf.size(); i++) {
                surf_file<<pts_surf[i].x<<"\t"<<pts_surf[i].y<<"\t";

		}

        surf_file<<"\n";
		for (i=0 ; i<prev_pts_surf.size(); i++) {
                surf_file<<prev_pts_surf[i].x<<"\t"<<prev_pts_surf[i].y<<"\t";
...
*/
		...
		for(int iScale = 0; iScale < scale_num; iScale++) {
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				if(show_track == 1 && iScale == 0)
					...
					DrawTrack(iTrack->point, iTrack->index, fscales[iScale], black);
		...
		if( show_track == 1 ) {
			...
			c = cvWaitKey(3);
			sprintf(liniec,"frame%d.jpg",contor_cadre);
			sprintf(linieb,"blank%d.jpg",contor_cadre);
			imwrite(liniec, image);
			imwrite(linieb, black);
			contor_cadre=contor_cadre+1;
			if((char)c == 27) break;
		}
	}
